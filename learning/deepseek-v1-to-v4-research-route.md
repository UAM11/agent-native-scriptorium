...
1. DeepSeek 先研究了 `batch size` 和 `learning rate` 的 scaling，而不是只研究“模型更大 + 数据更多”。
2. 它用 `non-embedding FLOPs/token` 来度量模型规模，而不是直接把参数量当全部答案。
3. 它明确指出数据质量会改变最优的 model/data allocation，所以 scaling law 不是脱离语料质量的常数。
4. 它没有把这些发现停留在理论层，而是立刻落到真实训练决策里，包括 `2T` bilingual corpus、`7B/67B`、`multi-step LR scheduler` 和 `SFT + DPO`。

#### 为什么它会自然导向 `V2`

第一阶段把“怎样把 dense 基础模型训对”这件事基本校准完了。  
一旦配比和训练规律相对清楚，下一步最显眼的瓶颈就不再是“会不会训”，而是：

- dense `FFN` 太贵
- attention 的 `KV cache` 太贵

所以第二阶段不是推翻第一阶段，而是顺着第一阶段继续往前走：
...
- [DeepSeek 第二阶段论文阅读笔记：DeepSeekMoE 与 DeepSeek-V2](deepseek-moe-and-v2-paper-reading.md)
