...
路线笔记会把这篇论文放在 `DeepSeek LLM / V1` 的起点；这篇笔记先只把起点本身讲清楚。
...
路线笔记只会说“DeepSeek 的起点是 scaling law”；这篇笔记把“为什么是 scaling law、作者自己怎么论证出来的”补完整。
...
路线笔记里“数据质量会改写 scaling law”这一点，在这里开始埋下前提。
...
路线笔记中的 `DeepSeek LLM / V1` 之所以仍然重要，不是因为架构花哨，而是因为它把后续大规模扩展的底座先搭稳了。
...
路线笔记里提到“第一阶段先把训练规律搞清楚”，这一节就是最直接的工程体现。
...
路线笔记里提到的“先研究 batch/lr scaling 再谈更大模型”，就是这里的核心贡献。
...
路线笔记中“DeepSeek 的第一阶段先把扩展规则做明白”，最核心的数学内容就在这一节。
...
路线笔记里“DeepSeek 重视数据工程和数据质量提升”，在理论上最强的支撑就来自这里。
...
路线笔记里后面会讲 `R1` 的 pure RL；这篇论文正好提供了它之前的对照基线。
...
路线笔记会把这部分简化成“V1 的 dense base 已经很强”；这里补的是它具体强在哪、为什么强。
...
路线笔记里后面会讲 `R1` 如何把“格式化推理”继续推进到“RL 驱动的 reasoning”；这里正好是前一个台阶。
...
路线笔记中的“第一阶段 chat 能力已经很强”在中文场景下主要由这里支撑。
...
路线笔记里“DPO 更偏改善 open-ended generation，而非 benchmark 分数”在这里最直观。
...
路线笔记里“DeepSeek 不是只追 benchmark 分数”，这里是很强的证据。
...
路线笔记后面如果讨论更强 agent 或 RL，这一节会成为一个很好的“基础安全基线”。
...
路线笔记会讲方法演化；这一节补的是“作者在第一阶段就已经有很多非常务实的训练判断”。
...
路线笔记之所以从 `DeepSeek LLM / V1` 自然走向 `V2 / R1 / V4`，这章已经提前给出线索。
